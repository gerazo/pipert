...
from src.constants import PACKET_RETRIEVED, EXECTION_TIME
...
class TimeToBufferAverageMeasurement(BaseChannelMeasurement):
    def _measure(self):
        for channel in self._channel_manager.get_channels():
            time_to_buffer = self.__calculate_channel_time_to_buffer_average(channel)
            channel.add_measure(self._measurement_key, 
                                [self._packet_cycle, time_to_buffer])
