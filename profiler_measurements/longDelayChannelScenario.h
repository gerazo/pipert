...
/// relatively longer processing time compared to the average processing  time of the other channels
    ...
    void run(const char *destination_uri)  {
        ...
        while(!kbhit()) {
            ...
            pipert::PolledChannel<measurements::Human> pc =
                    sch.CreatePolledChannel<measurements::Human>("OutChannel", channel_capacity);

            measurements::HumanReverser hr(&pc);
            pipert::ScheduledChannel<measurements::Human> sc2 =
                    sch.CreateScheduledChannel<measurements::Human>("ReverserChannel", channel_capacity, nullptr,
                                                                    std::bind(&measurements::HumanReverser::ReverseName,
                                                                              &hr, std::placeholders::_1));

            measurements::HumanPrinter hp(&sc2);
            pipert::ScheduledChannel<measurements::Human> sc1 =
                    sch.CreateScheduledChannel<measurements::Human>("PrinterChannel", channel_capacity, nullptr,
            ...
            for (int i = 0; i < 10; i++) {
                pipert::PacketToFill<measurements::Human> packet_to_fill = sc1.Acquire(time, "Jimi Hendrix", 28 + i);
            }
